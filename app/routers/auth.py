...
async def check_username(username_data: Dict[str, str] = Body(...)):
    ...
    if not username:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="用户名是必需的")
...
async def update_password(
    password_data: Dict[str, str] = Body(...), current_user: UserInDB = Depends(get_current_active_user)
):
    """修改当前用户的密码
    ...
    if not old_password or not new_password:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="旧密码和新密码都是必需的")
    ...
    await db.users.update_one({"_id": ObjectId(current_user.id)}, {"$set": {"hashed_password": hashed_password}})
...
async def update_username(
    username_data: Dict[str, str] = Body(...), current_user: UserInDB = Depends(get_current_active_user)
):
    """修改当前用户的用户名
    ...
    if not new_username:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="新用户名是必需的")
    ...
    # 更新用户名
    await db.users.update_one({"_id": ObjectId(current_user.id)}, {"$set": {"username": new_username}})
...
async def set_initial_password(
    ...
    if not new_password:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="新密码是必需的")

    # 检查用户是否已有密码
    if current_user.hashed_password:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="用户已有密码，请使用修改密码接口")
...
async def check_has_password(current_user: UserInDB = Depends(get_current_active_user)):
    """检查当前用户是否已设置密码
