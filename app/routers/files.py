...
# 解析文件格式限制
with open(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "file_formats.json"),
    "r",
    encoding="utf-8",
) as f:
...
async def upload_file(
    file: UploadFile = File(...), metadata: str = None, current_user: UserInDB = Depends(get_current_active_user)
):
    """
    上传3D模型文件
    ...
    if file_extension not in supported_extensions:
        raise HTTPException(status_code=400, detail=f"不支持的文件格式。支持的格式: {', '.join(supported_extensions)}")
    ...
    try:
        ...
        if file_extension == "glb":
            ...
            minio_client.put_object(
                SOURCE_BUCKET_NAME, file_path, file_stream, len(file_data), content_type=f"application/{file_extension}"
            )
            conversion = None
...
async def get_file(file_path: str, current_user: UserInDB = Depends(get_current_active_user)):
    """
    获取特定文件的详情和下载链接
    ...
    try:
        ...
        file_metadata = await db.files.find_one(
            {
                "file_path": file_path,
                "$or": [{"user_id": current_user.id}, {"is_public": True}, {"share_info.shared_with": current_user.id}],
            }
        )
        ...
        if file_metadata.get("conversion") and file_metadata["conversion"].get("output_file_path"):
            converted_url = minio_client.presigned_get_object(
                CONVERTED_BUCKET_NAME, file_metadata["conversion"]["output_file_path"]
            )
...
async def delete_file(file_id: str, current_user: UserInDB = Depends(get_current_active_user)):
    """
    删除文件
...
async def update_file(
    file_id: str, update_data: dict = Body(...), current_user: UserInDB = Depends(get_current_active_user)
):
    """
    更新文件信息
    ...
    await db.files.update_one({"_id": ObjectId(file_id)}, {"$set": db_update_data})
...
async def share_file(file_id: str, share_info: FileShare, current_user: UserInDB = Depends(get_current_active_user)):
    """
    分享文件
    ...
    await db.files.update_one({"_id": ObjectId(file_id)}, {"$set": {"share_info": share_info_dict}})
...
async def list_shared_files(current_user: UserInDB = Depends(get_current_active_user)):
    ...
    - **current_user**: 当前登录用户
    """
    files = await db.files.find({"share_info.shared_with": current_user.id}).to_list(length=None)
    return [FileMetadata(**file) for file in files]


@router.get("/download/{file_id}", response_model=dict)
async def get_download_url(file_id: str, current_user: UserInDB = Depends(get_current_active_user)):
    """
    根据文件ID获取下载链接
...
async def get_converted_download_url(file_id: str, current_user: UserInDB = Depends(get_current_active_user)):
    """
    根据文件ID获取转换后文件的下载链接
...
async def convert_file(
    file_id: str, output_format: Optional[str] = Form(None), current_user: UserInDB = Depends(get_current_active_user)
):
    """
    转换文件格式
    ...
    if output_format and output_format not in valid_formats:
        ...
        raise HTTPException(
            status_code=422, detail=f"无效的输出格式: {output_format}。支持的格式: {', '.join(valid_formats)}"
        )
    ...
    try:
        ...
        # 根据文件大小计算收费
        size_mb = file_metadata.get("file_size", 0) / (1024 * 1024)
        if size_mb > 200:
            raise HTTPException(status_code=400, detail="模型大小超过200MB限制")
        if size_mb <= 10:
            fee = 0
        elif size_mb < 100:
            fee = 5
            ...
            await db.transactions.insert_one(
                {
                    ...
                    "description": f"模型转换费用 {file_metadata.get('filename')}",
                    "created_at": datetime.now(),
                }
            )
        ...
        await db.files.update_one({"_id": ObjectId(file_id)}, {"$set": {"conversion": conversion.model_dump()}})
...
async def get_conversion_status(task_id: str, current_user: UserInDB = Depends(get_current_active_user)):
    """
    获取转换任务状态 (已弃用)
...
async def update_file_preview_image(
    file_id: str, data: dict = Body(...), current_user: UserInDB = Depends(get_current_active_user)
):
    """
    ...
    minio_client.put_object(
        PREVIEW_BUCKET_NAME, filename, buffer, length=buffer.getbuffer().nbytes, content_type="image/png"
    )
    ...
    await db.files.update_one(
        {"_id": ObjectId(file_id)}, {"$set": {"preview_image": preview_url, "updated_at": datetime.now()}}
    )
